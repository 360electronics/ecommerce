...
interface ProfileState {
  ...
  loadingStates: Partial<
    Record<"profile" | "orders" | "referrals" | "tickets", boolean>
  >;
  isRefetching: boolean;
  errors: Partial<
    Record<"profile" | "orders" | "referrals" | "tickets", AppError>
  >;
  lastFetched: Partial<
    Record<"profile" | "orders" | "referrals" | "tickets", number>
  >;
  fetchProfileData: (
    userId: string,
    force?: boolean,
  ...
  refetch: (
    section: "profile" | "orders" | "referrals" | "tickets",
    userId: string,
    force?: boolean,
    signal?: AbortSignal
  ) => Promise<void>;
...
export const useProfileStore = create<ProfileState>((set, get) => ({
  ...
  fetchProfileData: async (userId, force = false, signal) => {
    ...
    if (lastFetchedProfile && Date.now() - lastFetchedProfile < cacheDuration)
      return;
  ...
  fetchOrders: async (userId, force = false, signal) => {
    ...
    if (lastFetchedOrders && Date.now() - lastFetchedOrders < cacheDuration)
      return;
  ...
  fetchReferrals: async (userId, force = false, signal) => {
    ...
    if (
      lastFetchedReferrals &&
      Date.now() - lastFetchedReferrals < cacheDuration
    )
      return;
  ...
  fetchTickets: async (userId, force = false, signal) => {
    ...
    if (lastFetchedTickets && Date.now() - lastFetchedTickets < cacheDuration)
      return;
  ...
  shareCurrentPage: (section) => {
    if (typeof window === "undefined") {
      logError(
        "shareCurrentPage",
        new Error("Sharing not available in this environment")
      );
      return;
    }
    ...
    if (navigator.share) {
      navigator
        .share(shareData)
        .catch((error) => logError("shareCurrentPage", error));
    } else {
      navigator.clipboard
