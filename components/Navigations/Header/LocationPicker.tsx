...
const LocationPicker: React.FC<LocationPickerProps> = ({
  ...
  const fetchPincodeDetails = async (code: string) => {
    ...
    try {
      ...
      if (
        ...
        // If only one office → auto select
        if (offices.length === 1) {
          handlePostOfficeSelect(offices[0], code);
        } else {
  ...
  const handlePostOfficeSelect = (office: any, code: string) => {
    const districtName = office.District;
    const locationText = `${office.Name}, ${districtName}`;
  ...
  return (
    <div className="relative">
        ...
        <div className="flex flex-col items-start">
          {loading ? (
            ...
            <div className=" flex gap-1 md:flex-col text-start">
              <span className="text-[10px] md:text-xs font-medium max-w-[150px] truncate">
                {location || "Select Location"} {pincode && `- ${pincode}`}
              </span>
      ...
      {isOpen && !isMobile && (
        <div
          ref={modalRef}
          className="absolute top-full left-0 mt-2 w-72 bg-white rounded-lg shadow-lg z-50 p-4"
        >
                ...
                <div className="mt-2 max-h-40 overflow-y-auto border border-gray-200 rounded-lg bg-white shadow-sm">
                  {postOffices.map((office, index) => (
                    <button
                      key={index}
                      type="button"
                      onClick={() => handlePostOfficeSelect(office, pincode)}
