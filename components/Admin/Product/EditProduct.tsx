...
const DraggableSpecSection = ({
  ...
  const [{ isOver, canDrop }, drop] = useDrop({
    ...
    canDrop: (item: DragItem) =>
      !section.isFixed && item.id !== "general" && item.id !== "warranty",
  ...
  return (
    <div
      ref={ref}
      className={`rounded-md border p-4 transition-colors ${borderStyle} ${
...
export default function EditProductPage({ id }: { id: string }) {
  ...
  const [customAttributes, setCustomAttributes] = useState<{
    [variantId: string]: CustomAttribute[];
  }>({});
  ...
  const [categoryPresets, setCategoryPresets] =
  ...
  const variantImageInputRefs = useRef<{
    [variantId: string]: {
      main: React.RefObject<HTMLInputElement | null>;
      additional: (HTMLInputElement | null)[];
    };
  }>({});
          ...
          Array.isArray(data.variants) && data.variants.length > 0
            ? data.variants.map((variant: any, index: number) => {
                ...
                return {
                  ...
                  lowStockThreshold:
                    variant.lowStockThreshold?.toString() || "5",
        ...
        if (data.specifications && Array.isArray(data.specifications)) {
          ...
          data.specifications.forEach((spec: any, index: number) => {
            ...
            const existingSectionIndex = newSpecSections.findIndex(
              (section) =>
                section.name.toLowerCase() === spec.groupName.toLowerCase()
            );
            ...
            if (existingSectionIndex >= 0) {
              newSpecSections[existingSectionIndex].fields =
                fields.length > 0
                  ? fields
                  : newSpecSections[existingSectionIndex].fields;
            } else {
  ...
  useEffect(() => {
    ...
    setVariants((prev) =>
      prev.map((variant) => {
        const presetAttributes =
          categoryPresets[product.category]?.attributes || [];
    ...
    variants.forEach((variant) => {
      if (!newCustomAttributes[variant.id]) {
        const presetAttrNames =
          categoryPresets[product.category]?.attributes.map((a) => a.name) ||
          [];
  ...
  const addVariant = () => {
    ...
    const presetAttributes =
      (product.category && categoryPresets?.[product.category]?.attributes) ||
      [];
    const initialAttributes: Record<string, string | number | boolean> = {};
    presetAttributes.forEach((attr) => {
      initialAttributes[attr.name] =
        ...
        section.id === sectionId
          ? {
              ...
              fields: [
                ...section.fields,
                {
                  id: `field${section.fields.length + 1}-${Date.now()}`,
                  label: "",
                  value: "",
                },
              ],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
      ...
      const requiredAttributesFilled = presetAttributes
        ...
        .every(
          (attr) =>
            ...
            v.attributes[attr.name] !== undefined
        );
      return (
        v.name.trim() &&
        v.sku.trim() &&
    ...
    formData.append(
      ...
      product.subcategory &&
        categoryPresets &&
        categoryPresets[product.category]
        ? categoryPresets[product.category].subcategories.find(
    ...
    formData.append(
      "totalStocks",
      validVariants.reduce((sum, v) => sum + Number(v.stock), 0).toString()
    );
    ...
    const variantsPayload = validVariants.map((v) => {
      ...
      return {
        ...
        weight:
          v.weight && !isNaN(Number(v.weight))
      ...
      <div className="space-y-6 p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setProduct({ ...product, fullName: e.target.value })
                }
            ...
            <div>
              ...
              <div className="relative dropdown-container">
                ...
                >
                  <span>
                    {(product.category &&
                      categoryPresets[product.category]?.category.name) ||
                      "Select Category"}
                  </span>
            ...
            <div>
              ...
              <div className="relative dropdown-container">
                ...
                {openDropdown === "subcategory" && product.category && (
                  <div className="absolute z-10 mt-1 w-full rounded-md border bg-white shadow-lg">
                    {categoryPresets[product.category]?.subcategories.map(
                      (subcat) => (
                        <div
                          key={subcat.slug}
                          className="cursor-pointer px-4 py-2 text-sm hover:bg-gray-100"
              ...
              <div className="flex items-center space-x-2">
                <div className="relative dropdown-container flex-1">
                  ...
                  >
                    <span>
                      {product.brand
                        ? brands.find((b) => b.name === product.brand)?.name ||
                          "Select Brand"
                        : "Select Brand"}
                    </span>
                    <ChevronDown className="h-4 w-4 opacity-50" />
            ...
            <div>
              ...
              <div className="relative dropdown-container">
                ...
                >
                  <span>
                    {statusOptions.find((opt) => opt.value === product.status)
                      ?.label || "Select Status"}
                  </span>
                  <ChevronDown className="h-4 w-4 opacity-50" />
                </button>
                ...
                >
                  <span>
                    {deliveryModeOptions.find(
                      (opt) => opt.value === product.deliveryMode
                    ...
                    <div className="md:col-span-2">
                      <div className="flex flex-col items-center">
                        ...
                        >
                          {previewUrls[variant.id]?.[0] ? (
                            <div className="relative h-full w-full">
                              <img
                                ...
                                alt={`Main image for variant ${
                                  variantIndex + 1
                                }`}
                                className="object-contain p-2 h-full w-full aspect-square 0"
                              />
                              <button
                                type="button"
                          ...
                          <input
                            ref={
                              variantImageInputRefs.current[variant.id]?.main
                            }
                            type="file"
                            accept="image/*"
                            className="hidden"
                            onChange={(e) =>
                              handleImageUpload(variant.id, 0, e)
                            }
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
                        {Array.from({ length: 5 }).map((_, index) => (
                          <div
                            ...
                            onDrop={(e) =>
                              handleImageDrop(variant.id, index + 1, e)
                            }
                            onDragOver={(e) => e.preventDefault()}
                          >
                            {previewUrls[variant.id]?.[index + 1] ? (
                              <div className="relative h-full w-full">
                                <img
                                  ...
                                  className="object-contain p-2 h-full w-full aspect-square"
                                />
                                <button
                                  type="button"
                                  onClick={() =>
                                    removeImage(variant.id, index + 1)
                                  }
                                  className="absolute right-2 top-2 rounded-full bg-white p-1 shadow-md"
                                >
                                  <X className="h-4 w-4" />
                            ...
                            <input
                              ...
                              className="hidden"
                              onChange={(e) =>
                                handleImageUpload(variant.id, index + 1, e)
                              }
                            />
                          </div>
                        ))}
                  ...
                  <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        value={variant.mrp}
                        onChange={(e) =>
                          handleVariantChange(variant.id, "mrp", e.target.value)
                        }
                        type="number"
                        step="0.01"
                    ...
                    <div>
                      ...
                      <div className="relative dropdown-container">
                        <button
                          ...
                          className="flex w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm"
                          onClick={() =>
                            toggleDropdown(`weightUnit-${variant.id}`)
                          }
                        >
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        ...
                        <div className="relative dropdown-container">
                          <button
                            ...
                            className="flex w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm"
                            onClick={() =>
                              toggleDropdown(`dimensionUnit-${variant.id}`)
                            }
                          >
                      ...
                      {presetAttributes.map((attr) => (
                        <div key={attr.name}>
                          ...
                          {attr.type === "select" && attr.options ? (
                            ...
                            <Input
                              value={
                                variant.attributes[attr.name]?.toString() || ""
                              }
                              onChange={(e) => {
                                const value =
                                  attr.type === "number"
                      ...
                      {customAttributes[variant.id]?.map((attr, index) => (
                        <div key={index} className="flex items-end gap-2">
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              removeCustomAttribute(variant.id, index)
                            }
                            className="text-red-500 hover:text-red-700"
                          >
                            <X className="h-4 w-4" />
          ...
          <div className="flex justify-center">
            <Button
              type="submit"
              className="w-[40%] rounded-full bg-gradient-to-r from-[#ff6b00] to-[#ff9f00] hover:to-primary-hover"
            >
              Update Product
            </Button>
          </div>
