...
const ProductCardwithCart: React.FC<ProductCardProps> = ({
    ...
    async (e: React.MouseEvent) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          fetchWishlist();
          if (refetchProfile) {
            await refetchProfile('profile', userId);
          }
        } else {
  ...
  const handleCartClick = useCallback(
    ...
    [userId, productId, variantId, addToCart]
  );
      ...
      <div className="relative">
        ...
        {isHeartNeed && (
          <button
            ...
            className={cn(
              'absolute cursor-pointer right-12 bottom-0 z-10 p-2 rounded-full',
        ...
        <button
          ...
          className={cn(
            'absolute right-2 bottom-0 bg-black rounded-full p-2 text-white cursor-pointer',
            isAdding && 'opacity-50 cursor-not-allowed'
          )}
          disabled={isAdding}
