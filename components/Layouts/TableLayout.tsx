...
export function EnhancedTable<T extends Record<string, any>>({
  ...
  const isItemSelected = useCallback(
    (item: T) => {
      ...
      return selectedItems.some(
        (selectedItem) => selectedItem[selectionKey] === item[selectionKey]
      );
    },
  ...
  const toggleAllSelection = useCallback(() => {
    if (selectedItems.length === paginatedData.length) {
      ...
      const selectableItems = paginatedData.filter(
        (item) => !selection.disableSelection?.(item)
      );
  ...
  const renderCell = useCallback(
    (column: ColumnDefinition<T>, item: T) => {
      ...
      if (
        customization.cellRenderers &&
        customization.cellRenderers[String(column.key)]
      ) {
      ...
      return value !== undefined && value !== null ? (
        <div className="truncate max-w-[150px] text-sm">{String(value)}</div>
      ) : (
        ""
      );
    },
                  ...
                  .map((column) => (
                    <TableHead
                      ...
                      className={cn(
                        "py-4 px-6",
                        column.sortable && sorting.enabled
                          ? "cursor-pointer hover:bg-primary-light transition-colors duration-200"
                          : "",
                    ...
                    return (
                      <TableRow
                        ...
                        className={cn(
                          ...
                          customization.zebraStriping &&
                            index % 2 === 1 &&
                            "bg-gray-50",
                          customization.rowHoverEffect &&
